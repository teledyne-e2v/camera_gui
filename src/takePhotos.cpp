...
unsigned char *TakePhotos::TakePhotoInMemory() {
  unsigned char *frame =
      (unsigned char *)malloc(sizeof(unsigned char) * map->size);
...
void TakePhotos::TakePhotosInMemory() {
  ...
  frameCount++;
  if ((int)frameVect.size() ==
      numberOfPhotos) //  starts a new thread to save the frames.
  {
    std::thread savingFrameThread =
        std::thread(saveFrameFunction, &frameVect, imageWidth, imageHeight,
                    path, photoName);
...
void addIFD(unsigned char *Y800data, int *offset, short int tag,
            short int fieldType, int count, int valueOffset) {
...
void Y800toTIFF(unsigned char *Y800data, int row, int col, char *path,
                char *photoName, int photoNumber) {
  ...
  addIFD(tiffImage, &offset, 262, 3, 1,
         1 * 256 * 256); // PhotometricInterpretation
  ...
  std::string pathStart =
      std::string(path) + ((path[strlen(path) - 1] == '/') ? "" : "/") +
      std::string(photoName) + "_" + std::to_string(photoNumber + 1);
...
void saveFrameFunction(std::vector<unsigned char *> *frameVect, int width,
                       int height, char *path, char *photoName) {
  int i = 0;
