...
ModuleControl::ModuleControl(ModuleCtrl *moduleCtrl, bool multifocus)
    : moduleCtrl(moduleCtrl), multifocus(multifocus) {
...
void ModuleControl::apply() {
  ...
  if (PDAVariation != PDA && multifocus)
...
void ModuleControl::auto_controls_render() {
  ...
  for (auto &ctrl : controls) {
    ...
    if (strcmp(ctrl->control->type, "bool") == 0) {
      ...
      sprintf(tmp_name,"##%s",ctrl->control->name);
      ImGui::Checkbox(tmp_name, &ctrl->value_bool);
        
      
    } else {
      ImGui::Text("%s", ctrl->control->name);
      ImGui::SameLine(elementOffset);
      ImGui::InputInt(ctrl->control->name, &ctrl->value, 1, 100,
...
void ModuleControl::writeRegister() {
  ...
  ImGui::Text("Register (Hex):");
  ImGui::SameLine(elementOffset);
  ImGui::InputText("##wreg", registerToWrite, 7,
                   ImGuiInputTextFlags_CharsHexadecimal);

  ImGui::Text("Value (Hex):");
  ImGui::SameLine(elementOffset);
  ImGui::InputText("##regval", valueToWrite, 7,
                   ImGuiInputTextFlags_CharsHexadecimal);
...
void ModuleControl::setPDA(int PDA) { this->PDA = PDA; }
